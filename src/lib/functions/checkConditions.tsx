...
export default function checkConditions(unit: RawUnit): ConditionReport {
  ...
  for (let conditional of unit.conditionals) {
    ...
    if (success) {
      cr.evaluation[conditional.variable].action =
        conditional.onSuccess || defaultSuccessAction;
    } else {
      cr.evaluation[conditional.variable].action =
        conditional.onFail || defaultFailAction;
      cr.evaluation[conditional.variable].message =
        conditional.message || defaultMessage;
