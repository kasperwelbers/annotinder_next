...
export default function CoderView({ user }: CoderViewProps) {
  return (
    <StyledDiv>
      <h2>Coding jobs</h2>
      <CoderJobsTable user={user} />
    </StyledDiv>
  );
}

const CoderJobsTable = ({ user }: { user: MiddlecatUser }) => {
  ...
  const loadData = useCallback(
    async (query: DataQuery) => {
      let data = jobs.map((v) => {
        ...
        v.progress = v.n_total
          ? Math.round((100 * (v.n_coded || 0)) / v.n_total)
          : null;
